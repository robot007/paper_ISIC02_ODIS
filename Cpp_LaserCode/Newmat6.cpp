...
//	Check http://cc.usu.edu/~zhensong/fits/fitbrief.zip
//************************************************************
int BaseMatrix::Rank()
{
    REPORT
	...
	Mt=*this;
	SVD(Mt, D, U, V);
	int nRow,nCol,cnt,nRank;
	nRow=D.Nrows( );
	nCol=D.Ncols( );
	if(nRow!=nCol){
		Throw(Exception("Can't use SVD to find rank\n"));
		return -1;
	}
	nRank=0;
	for (cnt=1;cnt<=nRow;cnt++){
		if(fabs(D(cnt,cnt))>SMALLEST_POSITIVE){
// If the absolute value of elements on diagonal are not very near to 0, 
// rank counter plus 1.
// So far, I defined SMALLEST_POSITIVE as 1e-10 in "include.h"
// For engineering, this should be good enough, but it can't solve 
// theorically pathology system. 
// How to do?
			nRank++;
		}
	}
